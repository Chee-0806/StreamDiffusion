...
def denormalize(images: Union[torch.Tensor, np.ndarray]) -> torch.Tensor:
    ...
    """
    # Convert float16 to float32 for CPU operations (clamp not supported for float16 on CPU)
    if isinstance(images, torch.Tensor) and images.dtype == torch.float16 and not images.is_cuda:
        images = images.float()
